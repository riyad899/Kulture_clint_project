...
import { Menu, X, ChevronDown, Instagram, Mail, Phone } from 'lucide-react';
  ...
  return (
    <>
          ...
          <div className="flex justify-between items-center py-4">
            ...
            <div className="hidden lg:flex items-center space-x-6">
              {navItems.map((item) => (
                ...
                >
                  ...
                  {/* Active indicator */}
                  <div className={`absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-yellow-400 to-orange-500 transition-all duration-300 ${
            ...
            >
              <div className="relative w-6 h-6">
                <Menu className={`absolute inset-0 w-6 h-6 text-white transition-all duration-300 ${isOpen ? 'rotate-90 opacity-0' : 'rotate-0 opacity-100'}`} />
                <X className={`absolute inset-0 w-6 h-6 text-white transition-all duration-300 ${isOpen ? 'rotate-0 opacity-100' : '-rotate-90 opacity-0'}`} />
              </div>
            </button>
          </div>
        </div>
      ...
      <div className={`fixed inset-0 z-40 lg:hidden transition-all duration-500 ${
        ...
        {/* Menu Content */}
        <div className={`absolute top-0 right-0 h-full w-80 bg-gradient-to-br from-black via-gray-900 to-black border-l border-yellow-400/20 transform transition-transform duration-500 ${
